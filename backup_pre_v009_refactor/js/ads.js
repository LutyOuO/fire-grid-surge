...
var Ads = {
  ...
  timerId: 0,
  startedAt: 0,
  _wxAd: null,       // 微信激励视频广告实例
  _wxLoaded: false,  // 微信广告是否已加载
  _wxLoading: false, // 是否正在 load
  ...
  showRewarded: function (placement, onSuccess, onFail) {
    ...
    var self = this;
    this.timerId = setTimeout(function () { self.finishSuccess(); }, CONFIG.ADS.H5_DURATION_MS);
    Input.clearTap();
    return true;
  },
  ...
  _showWxAd: function (placement, onSuccess, onFail) {
    ...
    this.active = true;
    this.placement = placement;
    this._pendingShow = { onSuccess: onSuccess, onFail: onFail };
    Input.clearTap();
  ...
  getProgress: function () {
    ...
    return Math.max(0, Math.min(1,
      (Date.now() - this.startedAt) / CONFIG.ADS.H5_DURATION_MS));
  },
  ...
  draw: function (ctx) {
    ...
    var x = CONFIG.UI.AD_PANEL_X, y = CONFIG.UI.AD_PANEL_Y;
    var width = CONFIG.UI.AD_PANEL_WIDTH, height = CONFIG.UI.AD_PANEL_HEIGHT;
    ...
    var barX = x + 55, barY = y + 235, barWidth = width - 110;
