(function () {
  ...
  //   - 画布创建与 2D 上下文 / 屏幕尺寸 / DPR / 安全区
  //   - 触摸事件 / 本地存储 / 前后台生命周期 / 广告 / 音频
  //   - #57 UI 切图加载（loadImage）与微信胶囊矩形（getMenuButtonRect）
  // ============================================================
  ...
  var Platform = {
    ...
    // ---------- 初始化 ----------
    init: function () {
      this.isWx = typeof wx !== 'undefined' && wx && typeof wx.createCanvas === 'function';
      this.sysInfo = this.getSystemInfo();
      this._calcLayout();
      this.initCanvas();
      this.loadAssets();
      // 部分业务模块会在 Platform.init 之前登记图片。初始化平台后再补载一次，
      // 避免微信首帧误走浏览器 Image 分支而导致全部 PNG 永久为空。
      this.retryPendingImages();
    ...
    onTouchStart: function (cb) {
      this._touchHandlers.start.push(cb);
    },
    onTouchMove: function (cb) {
      this._touchHandlers.move.push(cb);
    },
    onTouchEnd: function (cb) {
      this._touchHandlers.end.push(cb);
    },
    onTouchCancel: function (cb) {
      this._touchHandlers.cancel.push(cb);
    },
    ...
    onKeyDown: function (cb) {
      this._keyDownHandlers.push(cb);
    },
    onKeyUp: function (cb) {
      this._keyUpHandlers.push(cb);
    },
    // ---------- 前后台生命周期 ----------
    onShow: function (cb) {
    ...
    imageFailed: {},
    imageSources: {},
    // 不能只看 isWx：脚本加载期 Platform.init 尚未执行，但 wx 已经存在。
    hasWxImageApi: function () {
      return typeof wx !== 'undefined' && wx && typeof wx.createImage === 'function';
    },
    // 资源路径：微信相对项目根，H5 相对 h5/index.html（需 ../ 前缀）
    assetPath: function (rel) {
      return this.isWx || this.hasWxImageApi() ? rel : '../' + rel;
    },
    loadImage: function (key, rel) {
      // 同一个资源在解码过程中不能重复创建，失败后才允许重试。
      if (this.imageSources[key] === rel && (this.imageReady[key] || this.imageLoading[key])) return this.images[key];
      this.imageSources[key] = rel;
      var src = this.assetPath(rel);
      var img = null;
      if (this.hasWxImageApi()) {
        try {
          img = wx.createImage();
        } catch (e) {
          img = null;
      ...
      if (!img) {
        this.imageReady[key] = false;
        this.imageFailed[key] = true;
        return null;
      }
      var self = this;
      this.imageReady[key] = false;
      this.imageFailed[key] = false;
      this.imageLoading[key] = true;
      img.onload = function () {
        self.imageLoading[key] = false;
        self.images[key] = img;
        self.imageReady[key] = true;
        self.imageFailed[key] = false;
      };
      img.onerror = function (error) {
        self.imageLoading[key] = false;
        self.imageReady[key] = false;
        self.imageFailed[key] = true;
        if (typeof console !== 'undefined' && console.warn) {
          console.warn('[ASSET LOAD FAILED]', key, src, error || 'unknown');
        }
      };
      try {
        img.src = src;
      } catch (e) { img.onerror(e); }
      // 某些环境 onload 已同步触发，先占位，成功后由 onload 覆盖
      this.images[key] = img;
      return img;
    },
    // 对预初始化阶段未能创建的图片进行一次可靠补载。
    retryPendingImages: function () {
      var keys = Object.keys(this.imageSources);
      for (var i = 0; i < keys.length; i++) {
        var key = keys[i];
        if (!this.imageReady[key] && !this.imageLoading[key]) this.loadImage(key, this.imageSources[key]);
      }
    },
    // 微信胶囊按钮矩形（CSS 像素）；H5 返回 null
    getMenuButtonRect: function () {
      if (!this.isWx || typeof wx.getMenuButtonBoundingClientRect !== 'function') return null;
      try {
    ...
    // 平台、画布就绪后统一创建图片，所有业务模块只查询资源状态。
    loadAssets: function () {
      var assets = root.CONFIG.ASSETS;
      for (var key in assets) this.loadImage(key, assets[key]);
    }
  };
